<template>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <div class="flex items-center gap-1.5">
          <span class="text-[11px] font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
            {{ analyticsStore.summary.pv_uv_delta || '0% 较昨日' }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-sm flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
      ...
      <div class="flex flex-col gap-1.5">
        <div class="text-xl font-bold text-[#0F172A] tracking-tight">
          {{ analyticsStore.summary.knowledge_units_count ?? 0 }}
          <span class="text-xs font-normal text-[#64748B] ml-0.5">篇</span>
          <span class="text-xs font-normal text-[#94A3B8] ml-1.5">({{ analyticsStore.summary.chunks_count ?? 0 }} 切片)</span>
        </div>
        <div class="text-[11px] text-[#64748B]">
          {{ analyticsStore.summary.units_synced ?? 0 }} 篇同步 / {{ analyticsStore.summary.units_pending ?? 0 }} 篇待更
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-sm flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
      ...
      <div class="flex flex-col gap-1.5">
        <div class="text-xl font-bold text-[#0F172A] tracking-tight font-mono">
          {{ typeof analyticsStore.summary.faq_cache_hit_rate === 'number' ? `${analyticsStore.summary.faq_cache_hit_rate}%` : (analyticsStore.summary.faq_cache_hit_rate || '0.0%') }}
        </div>
        <div class="flex items-center gap-1.5">
          <span class="text-[11px] font-medium text-[#0071E3] bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
            {{ analyticsStore.summary.tokens_saved || '节约 0 Token' }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-sm flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
      ...
      <div class="flex flex-col gap-1.5">
        <div class="text-xl font-bold text-[#0F172A] tracking-tight font-mono">
          {{ analyticsStore.summary.avg_latency_ms ?? 0 }}<span class="text-xs font-normal text-[#64748B] ml-0.5">ms</span>
        </div>
        <div class="text-[11px] text-[#64748B] font-mono">
          P95 {{ analyticsStore.summary.p95_latency_ms ?? 0 }}ms · P99 {{ analyticsStore.summary.p99_latency || '0ms' }}
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-sm flex flex-col justify-between gap-3 hover:border-blue-200 transition-colors">
      ...
      <div class="flex flex-col gap-1.5">
        <div class="text-xl font-bold text-[#0F172A] tracking-tight">
          {{ analyticsStore.summary.unresolved_gaps_count ?? 0 }}
        ...
        <div class="flex items-center gap-1.5">
          <span class="text-[11px] font-medium text-rose-700 bg-rose-50 px-2 py-0.5 rounded-full border border-rose-100">
            {{ analyticsStore.summary.unresolved_gaps_delta || '0个 本周新增' }}
          </span>
        </div>
      </div>
    </div>
